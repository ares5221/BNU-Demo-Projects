...
data_path = './../../data_split/chinese_split0409.xls'
base_dir = './../../data/preprocess_result_chinese'
normal_sheet_idx = 2
'''
...
def clean_ques_desc(ques_desc):
    ...
    '''
    # 1去除不合法字符
    ques_desc = ques_desc.replace('\n', '').replace('　', '').replace('','').replace('□','').replace('，', '').replace('．','').replace('（）','').replace('()','').replace(' ','').replace('_','').replace(',','').replace('。','').replace('“”','').replace('​​','').replace('（）','')
    # 2去除（   ）中文括号中零个或多个空格情况
...
if __name__ == '__main__':
    print('start preprocess chinese data...')
    get_data(data_path)
    print('end...')