...
def create_classifer_files(key):
    ...
    base_dir = './../crawler_data/target/'
    curr_dir = os.path.join(base_dir, key)
    if not os.path.exists(curr_dir):
        os.makedirs(curr_dir)
    if not os.path.exists(os.path.join(curr_dir, '教学设计')):
        os.makedirs(os.path.join(curr_dir, '教学设计'))
    if not os.path.exists(os.path.join(curr_dir, '备课方案')):
        os.makedirs(os.path.join(curr_dir, '备课方案'))
    if not os.path.exists(os.path.join(curr_dir, '教案')):
        os.makedirs(os.path.join(curr_dir, '教案'))
    if not os.path.exists(os.path.join(curr_dir, '方案')):
        os.makedirs(os.path.join(curr_dir, '方案'))
    if not os.path.exists(os.path.join(curr_dir, '上课方案')):
        os.makedirs(os.path.join(curr_dir, '上课方案'))
    if not os.path.exists(os.path.join(curr_dir, '执教方案')):
        os.makedirs(os.path.join(curr_dir, '执教方案'))
    if not os.path.exists(os.path.join(curr_dir, '教学PPT')):
        os.makedirs(os.path.join(curr_dir, '教学PPT'))
    if not os.path.exists(os.path.join(curr_dir, '课件')):
        os.makedirs(os.path.join(curr_dir, '课件'))
    if not os.path.exists(os.path.join(curr_dir, 'PPT')):
        os.makedirs(os.path.join(curr_dir, 'PPT'))
    if not os.path.exists(os.path.join(curr_dir, '练习题')):
        os.makedirs(os.path.join(curr_dir, '练习题'))
    if not os.path.exists(os.path.join(curr_dir, '随堂测试')):
        os.makedirs(os.path.join(curr_dir, '随堂测试'))
    if not os.path.exists(os.path.join(curr_dir, '学案')):
        os.makedirs(os.path.join(curr_dir, '学案'))
    if not os.path.exists(os.path.join(curr_dir, '导学案')):
        os.makedirs(os.path.join(curr_dir, '导学案'))
    if not os.path.exists(os.path.join(curr_dir, '任务单')):
        os.makedirs(os.path.join(curr_dir, '任务单'))
    if not os.path.exists(os.path.join(curr_dir, '讲义')):
        os.makedirs(os.path.join(curr_dir, '讲义'))
...
def save_corresponding_path(file_path, class_type, key_word):
    base_dir = './../crawler_data/target/'
    backup_dir = './../crawler_data/backup'
...
def get_content_type(file_name, content):
    ...
    for cc in content:
        full_text += cc
    if '教学设计' in file_name or '教学设计' in full_text or 'flash' in file_name:
        return '教学设计'
    elif '备课方案' in file_name or '备课方案' in full_text or 'mp3' in file_name:
        return '备课方案'
    elif '方案' in file_name or '方案' in full_text:
        return '方案'
    elif '教案' in file_name or '教案' in full_text or '诵读' in file_name:
        return '教案'
    elif '上课方案' in file_name or '上课方案' in full_text or '年级' in file_name:
        return '上课方案'
    elif '执教方案' in file_name or '执教方案' in full_text:
        return '执教方案'
    elif 'PPT' in file_name or 'PPT' in full_text:
        return 'PPT'
    elif '教学PPT' in file_name or '教学PPT' in full_text:
        return '教学PPT'
    elif '课件' in file_name or '课件' in full_text:
        return '课件'

    elif '练习题' in file_name or '练习题' in full_text:
        return '练习题'
    elif '随堂测试' in file_name or '随堂测试' in full_text:
        return '随堂测试'
    elif '学案' in file_name or '学案' in full_text:
        return '学案'
    elif '导学案' in file_name or '导学案' in full_text:
        return '导学案'
    elif '任务单' in file_name or '任务单' in full_text:
        return '任务单'
    else:
        return '讲义'
...
def phrase_zip(file_path):
    ...
    #     print(f_name)
    # zzz.close()
    class_type = get_content_type(file_name, '')
    print(class_type)
    # class_type = '讲义'
    save_corresponding_path(file_path, class_type, key_word)
    return
