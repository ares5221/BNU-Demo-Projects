...
class Agent(object):
    ...
    def state_to_representation_last(self, state):
        ...
        user_inform_slots_rep = np.zeros(len(self.slot_set.keys())-1)
        ...
        agent_inform_slots_rep = np.zeros(len(self.disease_symptom.keys()))
        try:
           agent_inform_slot = copy.deepcopy(state["agent_action"]["inform_slots"]["disease"])
           agent_inform_slots_rep[self.disease_symptom[agent_inform_slot]["index"]] = 1.0
        except:
            pass

        # Agent last request slot rep.
        agent_request_slots_rep = np.zeros(len(self.slot_set.keys())-1)
        try:
            ...
            pass
        #state_rep = np.hstack((current_slots_rep, user_action_rep, user_inform_slots_rep, agent_action_rep, agent_inform_slots_rep, agent_request_slots_rep, turn_rep))
        #这个state_rep是可疑改的，可疑根据具体要送到网络里什么信息来决定选那些变量拼进去
        state_rep = np.hstack((current_slots_rep, agent_inform_slots_rep, agent_request_slots_rep, turn_rep))
        # print('##19823###', state_rep.shape)
        return state_rep
    ...
    def _build_action_space(self):
        ...
        for slot in self.slot_set.keys():
            if slot != "disease":
                feasible_actions.append({'action': 'request', 'inform_slots': {}, 'request_slots': {slot: dialogue_configuration.VALUE_UNKNOWN}})
        # Diseases as actions.
        for disease in self.disease_symptom.keys():
            feasible_actions.append({'action': 'inform', 'inform_slots': {"disease":disease}, 'request_slots': {}})
    ...
    def disease_symptom_clip(self, disease_symptom, parameter):
        ...
        for key, value in disease_symptom.items():
            ...
            symptom_list = symptom_list[0:min(len(symptom_list), int(50))]
