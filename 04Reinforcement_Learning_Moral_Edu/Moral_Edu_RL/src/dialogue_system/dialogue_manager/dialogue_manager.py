...
class DialogueManager(object):
    ...
    def next(self,save_record,train_mode, greedy_strategy):
        ...
        :param train_mode: int, 1: the purpose of simulation is to train the model,
                                0: just for simulation and the parameters of the model will not be updated.
    ...
    def initialize(self,train_mode=1, epoch_index=None):
        self.trajectory = []
        self.state_tracker.initialize()   #初始化对话状态
        self.inform_wrong_disease_count = 0
        user_action = self.state_tracker.user.initialize(train_mode = train_mode, epoch_index=epoch_index)  #user初始化
        self.state_tracker.state_updater(user_action=user_action)
