...
class RunningSteward(object):
    ...
    def __init__(self, parameter, checkpoint_path):
        self.epoch_size = parameter.get("epoch_size",50)
    ...
    def simulate(self, agent, epoch_number, train_mode=1):
        """
        ...
        for index in range(0, epoch_number):
            ...
            result = self.evaluate_model(index, train_mode)
            if result["success_rate"] >= self.best_result["success_rate"] and \
                ...
                if save_model == 1:
                    ...
                    with open('best_result.txt',"a+", encoding='utf-8') as fp:
                        fp.write(str(index)+'\n')
                else:
                    pass
    ...
    def simulation_epoch(self, epoch_size, train_mode):
        ...
        for epoch_index in range(0, epoch_size):
    ...
    def evaluate_model(self,index, train_mode):
        """
        ...
        average_turn = float("%.3f" % (float(total_turns) / evaluate_epoch_number))
        ...
        # 存储log
        log_record = "%3d simulation SR %s, ABSR %s, ave reward %s, ave turns %s, ave wrong disease %s" % (index,res['success_rate'], res["ab_success_rate"],res['average_reward'], res['average_turn'], res["average_wrong_disease"])
        log_records_file_name = './../data/log.txt' #win10!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!
        # log_records_file_name = os.path.abspath('./') + '/src/dialogue_system/data/log.txt'#linux
        with open(log_records_file_name, 'a') as f:  # 设置文件对象
            f.write(log_record)
            f.write('\n')

        if train_mode == 0:
            result_record = 'simulationSR, ave reward, ave turns:' + str(res['success_rate']) + ' ' + str(res['average_reward']) + ' ' +\
                            str(res['average_turn']) + ' LocalTime:' + time.asctime(time.localtime(time.time()))
            records_file_name = './../data/test.txt' #win10!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!
            # records_file_name = os.path.abspath('./') + '/src/dialogue_system/data/test.txt'#linux
            with open(records_file_name, 'a') as f:  # 设置文件对象
