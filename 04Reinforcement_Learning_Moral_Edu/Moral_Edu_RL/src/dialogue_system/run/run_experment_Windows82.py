...
disease_number = 5
log_dir = "./../../../log/"
performance_save_path = "./../model/dqn/learning_rate04/"
checkpoint_path = "./../model/dqn/checkpoint/"
dialogue_file = "./../data/dialogue_output/dialogue_file.txt"
agent_id = 1
dqn_id = 1
warm_start = 1
explicit_number = 0
...
def run_train(simulate_epoch_number, epoch_size, evaluate_epoch_number, hidden_size_dqn, max_turn, reward_for_success,
    ...
    parameter['action_set'] = './../data/' + data_dir + 'action_set.p'
    parameter['slot_set'] = './../data/' + data_dir + 'slot_set.p'
    parameter['goal_set'] = './../data/' + data_dir + 'goal_set.p'
    parameter['disease_symptom'] = './../data/' + data_dir + 'disease_symptom.p'
...
if __name__ == "__main__":
    ...
    simulate_epoch_number = 20
    hidden_size_dqn = 64
    max_turn = 50
    with open('./../data/test.txt', 'a') as f:  # 设置文件对象
        f.write('Paramaters: ' + str(evaluate_epoch_number) + ', ' + str(epoch_size) + ', ' + str(
            warm_start_epoch_number) + ',| ' + str(hidden_size_dqn) + ', ' + str(
            reward_for_fail) + ', ' + str(
            simulate_epoch_number) + ', ' + str(reward_for_success) + ', ' + str(max_turn) + '\n')
        f.close()
        for i in range(experment_num):
            ...
            time_end = time.time()
            print('totally cost', time_end - time_start, '平均每轮耗时',(time_end - time_start) / 10)