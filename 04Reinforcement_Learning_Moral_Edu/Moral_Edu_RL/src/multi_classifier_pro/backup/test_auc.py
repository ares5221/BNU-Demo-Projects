...
def cal_auc_pro(y_test, y_score):
    n_classes = 5
    fpr = dict()
    tpr = dict()
    roc_auc = dict()
...
if __name__ == '__main__':
    ...
    '''
    for split_num in range(5,6):
        for i in range(1):
            # 读取进来的全部数据及对应label，shuffle后的结果629条
            trains, labels = process('records.dat', 'disease_tag.dat')
            #给不同切分比例设置相同的test数据
            # split train/test data set
            split = int(len(trains) * 0.5)
            data_test = trains[split:]
            id_test = labels[split:]
            data_train, id_train = get_train_data(trains, labels,split_num)
            print('-------------------------第 %d 组数据------------------------------------------' % i)
            LR(data_train, id_train, data_test, id_test)
            KNN(data_train, id_train, data_test, id_test)
            DT(data_train, id_train, data_test, id_test)
            SVM(data_train, id_train, data_test, id_test)
            NB(data_train, id_train, data_test, id_test)
